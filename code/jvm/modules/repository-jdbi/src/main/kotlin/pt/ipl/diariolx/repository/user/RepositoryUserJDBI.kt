...
import org.example.token.SessionToken
...
class RepositoryUserJDBI(
    ...
    override fun getTokenByTokenValidationInfo(tokenValidationInfo: SessionToken): Pair<User, Token>? =
        handle
            .createQuery(
                "select * from Users as users " +
                    "inner join Tokens as tokens on users.id = tokens.user_id " +
                    "where tokens.token_validation = :validation_information",
            ).bind("validation_information", tokenValidationInfo.value)
    ...
    override fun createToken(
        ...
        handle
            ...
            .bind("token_validation", token.sessionToken.value)
    ...
    override fun updateTokenLastUsed(
        ...
        handle
            ...
            .bind("validation_information", token.sessionToken.value)
            .execute()
    }

    override fun removeTokenByValidationInfo(tokenValidationInfo: SessionToken): Int =
        handle
            .createUpdate(
                """
                delete from Tokens
    ...
    private data class UserAndTokenModel(
        ...
        val tokenValidation: SessionToken,
        val balance: Balance,
        val createdAt: Long,
        val lastUsedAt: Long,
    ) {
...
private class UserAndTokenMapper : RowMapper<Pair<User, Token>> {
    override fun map(
        ...
        val token =
            Token(
                sessionToken = SessionToken(rs.getString("token_validation")),
                userId = rs.getInt("id"),
