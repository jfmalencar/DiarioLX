...
* On the `host` folder, `docker compose up -d --scale diariolx-jvm-dynamic=3` - scales the dynamic JVM service.
...
* `dig diariolx-jvm-dynamic` - uses `dig` to resolve the addresses for the `diariolx-jvm-dynamic` hostname.
