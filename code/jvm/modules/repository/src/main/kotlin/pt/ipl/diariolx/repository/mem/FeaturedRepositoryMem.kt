...
class FeaturedRepositoryMem(
    private val contentsById: (Int) -> ContentSummary?,
) : FeaturedRepository {
    ...
    override fun saveHomepage(sections: List<FeaturedSectionInput>) {
        this.sections.clear()
        currentId = 0
        sections.forEachIndexed { idx, input ->
            this.sections.add(
                FeaturedSection(
                    id = ++currentId,
                    type = input.type,
                    ...
                    position = idx,
                    contents = input.contentIds.mapNotNull { contentsById(it) },
                ),
            )
        }
    }

    override fun findPublishedIds(ids: List<Int>): Set<Int> = ids.filter { contentsById(it)?.publishedAt != null }.toSet()
}
