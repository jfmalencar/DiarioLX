...
\begin{tabular}{ll}
  {Supervisors:} & Artur Ferreira \\
                 ...
                 & Fátima Cardoso (ESCS)\\
\end{tabular}\\[10mm]
Beta report submitted in partial fulfilment of the requirements for\\
...
Colaborador, Editor e Administrador. Na versão beta, o fluxo editorial encontra-se
em desenvolvimento activo. A autenticação é implementada através de \textit{JSON
  Web Tokens} (JWT), com \textit{tokens} de acesso de curta duração e \textit{tokens} de atualização
de longa duração armazenados em \textit{cookies} \texttt{HttpOnly}, permitindo a validação
stateless dos pedidos e a mitigação de ataques XSS.
...
Administrator. In the beta version, the editorial workflow is under active development.
Authentication is implemented using JSON Web Tokens (JWT), with
short-lived access tokens and long-lived refresh tokens stored as \texttt{HttpOnly}
cookies, enabling stateless request validation while mitigating XSS attacks.
