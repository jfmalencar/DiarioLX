...
\begin{description}
      ...
      \item[Storage] Defines the storage interface for multimedia files. The concrete
            implementation, \texttt{Storage S3}, integrates with SeaweedFS~\cite{seaweedfs}
            for binary file storage using the AWS S3 SDK~\cite{aws_s3_sdk}, as SeaweedFS
            exposes an S3-compatible API. SeaweedFS was chosen as it can be self-hosted at
            no cost, which was a requirement given the project's academic and non-commercial
            nature.
...
which encapsulates all JWT logic. Access tokens are signed with HMAC-SHA256 using a
configurable secret key loaded at startup via \texttt{JwtConfig}. The signing key is
initialised lazily and reused across requests. Refresh tokens are generated as
...
\section{Authorization} \label{sec:be_authorisation}
...
\subsection{Transaction Management} \label{sec:be_transactions}
...
The API is documented using the OpenAPI standard, generated automatically by
\texttt{springdoc-openapi}~\cite{springdoc} and served via Scalar~\cite{scalar},
a modern API reference UI. Custom annotations (\texttt{@MayReturnNotFound},
\texttt{@MayReturnBadRequest}, etc.) were defined to reduce boilerplate in
controller declarations while maintaining accurate schema documentation. The
documentation is available at \texttt{/docs} when the application is running.

\section{Tests} \label{sec:be_tests}

Testing is organised by module, with dedicated test suites for the repository,
