...
outlet. The system is organised around two distinct areas:
...
\begin{itemize}
      ...
      \item \textbf{Public Website} - a reader-facing interface providing access to published articles, multimedia content (podcasts, videos, and photo essays), organised by category, tag, and editorial section.
\end{itemize}

The system is built around an editorial workflow in which content progresses through
defined states — draft, pending review, approved, and rejected — with each transition
guarded by the requesting user's role.

\section{Report Organisation} \label{sec:intro_organization}
...
The report is complemented by three appendices:

\begin{itemize}
      \item Appendix~\ref{app:db_model} provides the complete data model --- the full entity-relationship diagram, the physical database schema, and the enumerated types, views, and indexes that support it.
      \item Appendix~\ref{app:deployment} presents the deployment diagram of the production environment.
      \item Appendix~\ref{app:screenshots} collects screenshots of the backoffice and public-website interfaces.
\end{itemize}
