...
\end{lstlisting}
  ...
  \item[\textit{Staging}] Todos os serviços — \textit{backend}, \textit{frontend},
        base de dados, \textit{SeaweedFS} e \textit{Nginx} — são executados em \textit{containers}
        \textit{Docker}. As variáveis de ambiente são carregadas a partir do
...
Alterar o ficheiro \texttt{code/jvm/.env.staging} com o seguinte conteúdo:
...
Criar o ficheiro \texttt{code/jvm/seaweed-config.json} com o seguinte conteúdo:
